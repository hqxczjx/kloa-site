...
// 限流上限固定在 wrangler.jsonc 的 ratelimits binding（生成端点共享桶 10/60s、video-status 轮询 60/60s），
// 此处仅保留窗口时长：binding 响应无重置时刻，429 的 Retry-After 保守取整窗（须与 binding 的 period 一致）
export const RATE_LIMIT_WINDOW_SEC = 60;              // 生成端点窗口（秒）
...
// video-status 轮询端点独立限流（不复用生成端点的 10/60s 桶：客户端固定 5s 轮询会打爆共享桶）
export const VIDEO_STATUS_RATE_LIMIT_WINDOW_SEC = 60; // 轮询端点窗口（秒），上限 60/60s 在 wrangler.jsonc
